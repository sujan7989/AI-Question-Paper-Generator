...
export function Analytics() {
  ...
  const handleSendPasswordReset = useCallback(async (email: string, name: string) => {
    try {
      const { error } = await supabase.auth.resetPasswordForEmail(email, {
        redirectTo: `${window.location.origin}/reset-password`,
      });
      if (error) throw error;
      toast({ title: "Reset Email Sent", description: `Password reset link sent to ${name}.` });
    } catch {
      toast({ title: "Failed", description: "Could not send reset email.", variant: "destructive" });
    }
  }, [toast]);

  // Download all papers for a specific staff member
  const handleDownloadStaffPapers = useCallback((staffUserId: string, staffName: string) => {
    const staffPapers = generatedPapers.filter(p => p.user_id === staffUserId);
    if (staffPapers.length === 0) {
      toast({ title: "No papers", description: `${staffName} has no generated papers.`, variant: "destructive" });
      return;
    }
    const rows = staffPapers.map((p, i) => {
      const subj = subjects.find(s => s.id === p.subject_id);
      return `<tr><td>${i + 1}</td><td>${subj?.subject_name || 'Unknown'}</td><td>${subj?.course_code || 'N/A'}</td><td>${p.total_marks || 'N/A'}</td><td>${p.paper_config?.difficulty || 'medium'}</td><td>${new Date(p.created_at).toLocaleDateString()}</td></tr>`;
    }).join('');
    const html = `<!DOCTYPE html><html><head><title>${staffName} Papers</title><style>body{font-family:Arial,sans-serif;margin:20mm;}table{width:100%;border-collapse:collapse;}th,td{border:1px solid #000;padding:8px;}th{background:#e8e8e8;}h2{text-align:center;}</style></head><body><h2>${staffName} — Generated Papers</h2><table><tr><th>#</th><th>Subject</th><th>Code</th><th>Marks</th><th>Difficulty</th><th>Date</th></tr>${rows}</table><p style="font-size:9pt;color:#555;margin-top:16px;">Generated by QuestionCraft AI on ${new Date().toLocaleString()}</p></body></html>`;
    const w = window.open('', '_blank');
    if (w) { w.document.write(html); w.document.close(); setTimeout(() => w.print(), 500); }
  }, [generatedPapers, subjects, toast]);
                  ...
                  return (
                    <div key={staffUser.user_id} className="flex items-center justify-between p-3 border rounded-lg hover:bg-muted/50 transition-colors">
                      ...
                      <div className="flex items-center gap-4 text-sm">
                        ...
                        <Button variant="ghost" size="sm" title="Send password reset" onClick={() => handleSendPasswordReset(staffUser.email, `${staffUser.first_name} ${staffUser.last_name}`)}>
                          <KeyRound className="w-3.5 h-3.5 text-muted-foreground" />
                        </Button>
                        <Button variant="ghost" size="sm" title="Download papers report" onClick={() => handleDownloadStaffPapers(staffUser.user_id, `${staffUser.first_name} ${staffUser.last_name}`)}>
